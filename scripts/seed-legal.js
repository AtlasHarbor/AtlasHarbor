...
try{const text=await fs.readFile(path.join(root,'.env'),'utf8');for(const raw of text.split(/\r?\n/)){const line=raw.trim();if(!line||line.startsWith('#'))continue;const i=line.indexOf('=');if(i<1)continue;const name=line.slice(0,i).trim(),value=line.slice(i+1).trim().replace(/^(['"])(.*)\1$/,'$2');if(!(name in process.env))process.env[name]=value}}catch{}
const dir=path.join(root,'data/legal/cases');
const url=process.env.SUPABASE_URL?.replace(/\/$/,'');
const key=process.env.SUPABASE_SECRET_KEY;
if(!url||!key){console.error('SUPABASE_URL and SUPABASE_SECRET_KEY are required in .env or the process environment.');process.exit(1)}
...
for(const file of files){const c=JSON.parse(await fs.readFile(path.join(dir,file),'utf8'));const now=new Date().toISOString();
  ...
  await upsert('legal_case_events',(c.timeline||[]).map((e,i)=>({case_slug:c.slug,event_key:`${e.date||'unknown'}-${i}`,event_date:e.date||null,event_type:e.type||'procedural',description:e.event||e.description||'',source_url:e.sourceUrl||null,record:e})),'case_slug,event_key');
  await upsert('legal_case_sources',(c.sources||[]).map((s,i)=>({case_slug:c.slug,source_key:`${i}-${s.url}`,title:s.title||s.url,url:s.url,source_type:s.type||s.sourceType||'secondary',verified_at:s.verifiedAt||null,record:s})),'case_slug,source_key');
