...
let map=null,networkLayer=null,facilityLayer=null,vehicleLayer=null,markers=new Map(),selected=null,timer=null;

function pathDistance(path){let d=0;for(let i=1;i<path.length;i++){const a=path[i-1],b=path[i],dx=(b[1]-a[1])*Math.cos((a[0]+b[0])*Math.PI/360),dy=b[0]-a[0];d+=Math.hypot(dx,dy)}return d}
...
function start(target){if(!target||map===target)return;map=target;renderStatic();installLegend();clearInterval(timer);timer=setInterval(updatePositions,850);window.addEventListener('pagehide',()=>clearInterval(timer),{once:true})}
