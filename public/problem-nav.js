...
const spaces = [
  ...
  ['/game', 'Logistics Control Tower', 'Operate plants, orders, inventory, and transportation.'],
...
if (currentNav) {
  ...
  const close = () => {
    root.classList.remove('open', 'hover-open');
  ...
  if (matchMedia('(hover: hover) and (pointer: fine) and (min-width: 761px)').matches) {
    ...
    root.addEventListener('pointerleave', () => {
      cancelHoverClose();
      hoverCloseTimer = setTimeout(() => root.classList.remove('hover-open'), 180);
    });
...
if (matchMedia('(hover: none), (pointer: coarse)').matches) {
  ...
  document.addEventListener('touchmove', (event) => {
    ...
    const touch = event.touches[0];
    if (Math.abs(touch.clientX - start.x) > 12 || Math.abs(touch.clientY - start.y) > 12) moved = true;
  }, { passive: true });
  document.addEventListener('touchend', (event) => {
    if (!start || moved) { start = null; return; }
    const changed = event.changedTouches[0];
    const target = (changed && document.elementFromPoint(changed.clientX, changed.clientY)) || start.target;
    const link = target?.closest?.('a[href]');
    start = null;
    if (!link || link.hasAttribute('data-no-touch-nav') || link.hasAttribute('download') || link.closest('[contenteditable="true"]') || link.getAttribute('role') === 'button') return;
    const href = link.getAttribute('href') || '';
    if (!href || href.startsWith('javascript:')) return;
    setTimeout(() => {
      if (link.target === '_blank') window.open(link.href, '_blank', 'noopener');
      else if (location.href !== link.href) location.assign(link.href);
    }, 0);
  }, { passive: true });
}
