...
const POINTS={
 'la-port':{name:'Los Angeles Import Gateway',lat:33.74,lng:-118.27},
 lax:{name:'LAX Air Cargo Gateway',lat:33.94,lng:-118.4},
 'anaheim-pack':{name:'Anaheim Packaging Studio',lat:33.84,lng:-117.91},
 'riverside-plant':{name:'Riverside Contract Manufacturing',lat:33.98,lng:-117.37},
 'seattle-print':{name:'Seattle Print & Media Plant',lat:47.59,lng:-122.32},
 'savannah-port':{name:'Savannah Import Gateway',lat:32.08,lng:-81.09},
 'mem-air':{name:'Memphis Recovery Hub',lat:35.04,lng:-89.98},
 'dallas-dc':{name:'Dallas Retail Distribution Center',lat:32.78,lng:-96.8},
 'atlanta-dc':{name:'Atlanta E-commerce Fulfillment Center',lat:33.75,lng:-84.39},
 'allentown-dc':{name:'Allentown Marketplace Fulfillment Center',lat:40.61,lng:-75.49}
};
...
const cache=new Map();
const key=(mode,from,to)=>`${mode}:${from}:${to}`;
const rad=value=>value*Math.PI/180;
function haversine(a,b){const dLat=rad(b[0]-a[0]),dLng=rad(b[1]-a[1]),x=Math.sin(dLat/2)**2+Math.cos(rad(a[0]))*Math.cos(rad(b[0]))*Math.sin(dLng/2)**2;return 6371000*2*Math.atan2(Math.sqrt(x),Math.sqrt(1-x))}
...
function fallback(mode,from,to){const a=POINTS[from],b=POINTS[to];if(!a||!b)return null;let coordinates;if(mode==='truck')coordinates=reverseLookup(ROAD_CORRIDORS,from,to);else if(mode==='rail')coordinates=reverseLookup(RAIL_CORRIDORS,from,to);else if(mode==='ocean')coordinates=reverseLookup(OCEAN_CORRIDORS,from,to);else coordinates=arc(from,to);coordinates=coordinates||arc(from,to,18);return{mode,from,to,coordinates,distanceMeters:distance(coordinates),durationSeconds:null,source:mode==='truck'?'atlas-road-fallback':mode==='rail'?'atlas-rail-corridor':mode==='ocean'?'atlas-ocean-corridor':'atlas-air-arc'}}
async function osrmRoute(fetchImpl,from,to){const a=POINTS[from],b=POINTS[to];if(!a||!b)return null;const url=`https://router.project-osrm.org/route/v1/driving/${a.lng},${a.lat};${b.lng},${b.lat}?overview=full&geometries=geojson&steps=false`;const response=await fetchImpl(url,{headers:{Accept:'application/json','User-Agent':'AtlasHarbor/1.0'},signal:AbortSignal.timeout(12000)});if(!response.ok)throw new Error(`OSRM returned ${response.status}`);const data=await response.json(),route=data.routes?.[0],raw=route?.geometry?.coordinates;if(!route||!Array.isArray(raw)||raw.length<2)throw new Error('OSRM returned no route geometry.');return{mode:'truck',from,to,coordinates:raw.map(([lng,lat])=>[lat,lng]),distanceMeters:route.distance,durationSeconds:route.duration,source:'osrm-openstreetmap'}}
async function routeFor(fetchImpl,mode,from,to){const cacheKey=key(mode,from,to),saved=cache.get(cacheKey);if(saved&&Date.now()-saved.savedAt<24*3600000)return saved.value;let value;if(mode==='truck'){try{value=await osrmRoute(fetchImpl,from,to)}catch{value=fallback(mode,from,to)}}else value=fallback(mode,from,to);if(value)cache.set(cacheKey,{savedAt:Date.now(),value});return value}
