...
export function createApp({mlb=createMlbClient(),legal=createLegalService(),baseballCache=createBaseballCache()}={}){
 const app=express();
 ...
 app.get("/api/baseball/teams/:id",async(req,res)=>{if(!/^\d+$/.test(req.params.id))return res.status(400).json({error:"Invalid team ID."});try{const result=await getTeam(req.params.id);return result.data?res.json({team:result.data,cache:result.cache,fetchedAt:result.fetchedAt}):res.status(404).json({error:"Team not found."})}catch(error){console.error(error);return res.status(502).json({error:"Team statistics are temporarily unavailable."})}});
 app.get("/api/baseball/players",async(req,res)=>{const query=String(req.query.q??"").trim();if(query.length<2)return res.json({players:[]});try{const found=await mlb.search(query);return res.json({players:found.filter(item=>item.type==="player").map(item=>({id:item.id,name:item.name,team:item.subtitle.split(" · ")[0],position:item.subtitle.split(" · ")[1]}))})}catch(error){console.error(error);return res.status(502).json({error:"Player search is temporarily unavailable."})}});
 app.get("/api/baseball/players/:id",async(req,res)=>{if(!/^\d+$/.test(req.params.id))return res.status(400).json({error:"Invalid player ID."});try{const result=await getPlayer(req.params.id);return result.data?res.json({player:result.data,cache:result.cache,fetchedAt:result.fetchedAt}):res.status(404).json({error:"Player not found."})}catch(error){console.error(error);return res.status(502).json({error:"Player statistics are temporarily unavailable."})}});
 app.get("/api/legal/cases",async(_req,res)=>{try{return res.json({cases:await legal.listCases()})}catch(error){console.error(error);return res.status(500).json({error:"Legal cases are temporarily unavailable."})}});
 app.get("/api/legal/cases/:slug",async(req,res)=>{try{const item=await legal.getCase(req.params.slug);return item?res.json(item):res.status(404).json({error:"Case not found."})}catch(error){console.error(error);return res.status(500).json({error:"Case record is temporarily unavailable."})}});
 app.post("/api/legal/cases/:slug/refresh",async(req,res)=>{if(!process.env.LEGAL_ADMIN_TOKEN||req.get("authorization")!==`Bearer ${process.env.LEGAL_ADMIN_TOKEN}`)return res.status(401).json({error:"Unauthorized."});try{const proposal=await legal.refreshCase(req.params.slug);return proposal?res.json({proposal}):res.status(404).json({error:"Case not found."})}catch(error){console.error(error);return res.status(502).json({error:error.message})}});
 app.get(["/baseball/teams/:id","/baseball/teams/:id/:slug"],async(req,res)=>{if(!/^\d+$/.test(req.params.id))return res.status(404).send("Team not found");try{const result=await getTeam(req.params.id),team=result.data;if(!team)return res.status(404).send("Team not found");const slug=teamSlug(team),base=(process.env.PUBLIC_APP_URL||`${req.protocol}://${req.get("host")}`).replace(/\/$/,""),canonicalPath=`/baseball/teams/${team.id}/${slug}`;if(req.path!==canonicalPath)return res.redirect(301,canonicalPath);res.set("Cache-Control","public,max-age=300,stale-while-revalidate=86400");return res.type("html").send(renderBaseballTeamPage(team,`${base}${canonicalPath}`,result))}catch(error){console.error(error);return res.status(503).send("Team temporarily unavailable")}});
 app.get(["/baseball/games/:id","/baseball/games/:id/:slug"],async(req,res)=>{if(!/^\d+$/.test(req.params.id))return res.status(404).type("html").send(renderGameNotFoundPage());try{const result=await getGame(req.params.id);let game=result.data;if(!game)return res.status(404).type("html").send(renderGameNotFoundPage());game=await enrichGame(game);const slug=gameSlug(game),base=(process.env.PUBLIC_APP_URL||`${req.protocol}://${req.get("host")}`).replace(/\/$/,""),canonicalPath=`/baseball/games/${game.id}/${slug}`;if(req.path!==canonicalPath)return res.redirect(301,canonicalPath);res.set("Cache-Control","public,max-age=60,stale-while-revalidate=3600");return res.type("html").send(renderBaseballGamePage(game,`${base}${canonicalPath}`))}catch(error){console.error(error);return res.status(503).type("html").send(renderGameNotFoundPage())}});
 ...
 app.get(["/published","/published/{*path}"],(_req,res)=>res.sendFile(page("published.html")));
 app.get(["/problems","/problems/{*path}"],(_req,res)=>res.sendFile(page("problems.html")));
 app.get(["/admin","/admin/{*path}"],(_req,res)=>res.sendFile(page("admin.html")));
