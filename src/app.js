...
export function createApp({ mlb = createMlbClient() } = {}) {
  ...
  app.get("/api/baseball/players", async (req,res)=>{const q=String(req.query.q??"").trim();if(q.length<2)return res.json({players:[]});try{const results=await mlb.search(q);return res.json({players:results.filter(i=>i.type==="player").map(i=>({id:i.id,name:i.name,team:i.subtitle.split(" · ")[0],position:i.subtitle.split(" · ")[1]}))})}catch(e){console.error(e);return res.status(502).json({error:"Player search is temporarily unavailable."})}});
  ...
  app.get("/{*path}",(_req,res)=>res.sendFile(path.join(directory,"../public/index.html")));
  return app;
}
