...
async function parse(response){const text=await response.text();let data=null;try{data=text?JSON.parse(text):null}catch{}if(!response.ok){const error=new Error(data?.message||data?.error||text||`Request failed (${response.status})`);error.status=response.status;throw error}return data}
...
function createSupabaseStore(env,fetchImpl){
 ...
 async function writeForUser(token,userId,config){
  ...
  await parse(await fetchImpl(`${base}/rest/v1/game_progress`,{method:'POST',headers:{...userHeaders(token),Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({user_id:userId,state,updated_at:new Date().toISOString()})}));
  return config;
 }
...
export function createAdminControl({env=process.env,fetchImpl=globalThis.fetch}={}){
 ...
 router.post('/api/admin/run-quality',route(async(req,res)=>{const{token,current,config}=await verify(req),ai={...defaultAI,...config.ai};const result=await runAIHealthCheck({fetchImpl,providers:providers(ai),instruction:ai.instructionSet});await save(token,current.id,{...config,ai:{...ai,lastRunAt:new Date().toISOString(),lastError:result.ok?null:result.reason,lastRunResult:result}});res.json(result)}));
