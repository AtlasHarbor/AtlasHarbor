...
export function createPublishedFeedRouter({env=process.env,fetchImpl=globalThis.fetch}={}){
 ...
 async function collect(req,{token=null}={}){
  ...
  if(secret){try{const response=await fetchImpl(`${base}/auth/v1/admin/users?per_page=1000`,{headers:{apikey:secret,Authorization:`Bearer ${secret}`}}),data=await readJson(response);for(const account of data?.users||[])appendAccount(account)}catch(error){console.warn('Published metadata feed unavailable:',error.message)}}
  else{const bearer=String(req.get('authorization')||'').replace(/^Bearer\s+/i,'');if(bearer)try{const response=await fetchImpl(`${base}/auth/v1/user`,{headers:{apikey:publishable,Authorization:`Bearer ${bearer}`}});if(response.ok)appendAccount(await readJson(response))}catch(error){console.warn('Current user publication feed unavailable:',error.message)}}
  return rows;
 }
 router.get('/api/published-feed',async(req,res)=>{const combined=await collect(req),unique=new Map();for(const row of combined){const key=row.share_token||row.id;if(key)unique.set(key,safeSummary(row))}const publications=[...unique.values()].sort((a,b)=>new Date(b.published_at||b.updated_at||0)-new Date(a.published_at||a.updated_at||0)).slice(0,250);res.set('Cache-Control','public,max-age=15,stale-while-revalidate=60');res.json({publications,source:secret?'database-and-account-metadata':'database-and-current-account'})});
