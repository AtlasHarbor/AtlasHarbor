...
export function createEconomicsService({storage,env=process.env,fetchImpl=globalThis.fetch}={}){
 ...
 async function execute(runId,trigger){
  try{
   ...
   const result=await runEconomicsNow({fetchImpl,settings:economics,providers:providers(current.config,env),instruction:economics.ai_instruction,existingProblems:economics.problems||[],researchProvider:researchProvider(current.config,env),onHeadlines:async(baseline,counts)=>{await storage.updateHost(metadata=>{const admin={...(metadata.atlas_admin||{})},latest={...DEFAULT_ECONOMICS,...(admin.economics||{})};admin.economics={...latest,problems:mergeProblems(latest.problems||[],baseline),current_run:latest.current_run?.runId===runId?{...latest.current_run,status:'enriching',headlineCount:baseline.length,...counts}:latest.current_run};return{...metadata,atlas_admin:admin}})}}),finishedAt=new Date().toISOString();
