...
export function createWorkspaceRouter({env=process.env,fetchImpl=globalThis.fetch,storage=createProblemSpaceStorage({env,fetchImpl})}={}){
 ...
 router.post('/api/workspaces-form/:resourceType/:resourceId',async(req,res)=>{
  const requestId=text(req.body?.request_id,120);
  try{
   ...
   formReply(res,{type:'atlas-workspace-form-result',requestId,ok:true,...result},200);
  }catch(error){
   console.error('Workspace form fallback:',error);
   formReply(res,{type:'atlas-workspace-form-result',requestId,ok:false,error:error.message||'Workspace form save failed.'},error.status||500);
  }
 });
 return router;
}
