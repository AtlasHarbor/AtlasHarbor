...
export function createMlbClient(fetchImpl = globalThis.fetch) {
  async function request(path) {
    const response = await fetchImpl(`${MLB_BASE_URL}${path}`, { headers: { Accept: "application/json", "User-Agent": "AtlasHarbor/0.3" }, signal: AbortSignal.timeout(12_000) });
  ...
  return {
    ...
    async getTeam(id) {
      const [teamData, hitting, pitching, fielding, active, injured, nextGames, pinch] = await Promise.all([
        ...
        request(`/teams/${id}/roster?rosterType=active&hydrate=person(stats(group=[hitting,pitching],type=[season]))`),
        request(`/teams/${id}/roster?rosterType=injuredList&hydrate=person`),
      ...
      if (!team) return null;
      const roster = (active.roster ?? []).map(normalizeRosterPlayer);
      const nextGame = nextGames.dates?.flatMap((d) => d.games ?? [])[0];
      let lineup = [], bench = [];
      if (nextGame) {
        try {
          const box = await request(`/game/${nextGame.gamePk}/boxscore`);
          const side = Number(nextGame.teams?.home?.team?.id) === Number(id) ? "home" : "away";
          const teamBox = box.teams?.[side] ?? {};
          lineup = (teamBox.battingOrder ?? []).map((pid) => normalizeBoxPlayer(teamBox.players?.[`ID${pid}`])).filter(Boolean);
          bench = (teamBox.bench ?? []).map((pid) => normalizeBoxPlayer(teamBox.players?.[`ID${pid}`])).filter(Boolean);
        } catch { /* Pregame boxscore can be unavailable. */ }
      }
      const pinchById = new Map(pinch.map((split) => [split.player?.id, split.stat ?? {}]));
      return {
        id: team.id, name: team.name, abbreviation: team.abbreviation, league: team.league?.name, division: team.division?.name, venue: team.venue?.name, firstYear: team.firstYearOfPlay,
        ...
        injuredList: (injured.roster ?? []).map((entry) => ({ ...normalizeRosterPlayer(entry), status: entry.status?.description ?? entry.status?.code ?? "Injured list" })),
    ...
    async getPlayer(id) {
      ...
      const [data, gameLog, splits, transactions] = await Promise.all([
        ...
        request(`/people/${id}/stats?stats=gameLog&group=hitting,pitching,fielding&season=${season()}&hydrate=team,opponent`),
        request(`/people/${id}/stats?stats=statSplits&group=hitting,pitching&season=${season()}&sitCodes=vl,vr,home,away`),
        request(`/transactions?playerId=${id}&startDate=${season()}-01-01&endDate=${dateOnly(new Date())}`),
      ]);
      const player = data.people?.[0];
      if (!player) return null;
      const logs = gameLog.stats?.flatMap((entry) => entry.splits ?? []) ?? [];
      return {
        ...
        number: player.primaryNumber ?? null, bats: player.batSide?.description ?? null, throws: player.pitchHand?.description ?? null, age: player.currentAge ?? null, birthDate: player.birthDate ?? null, birthCity: player.birthCity ?? null, birthCountry: player.birthCountry ?? null, height: player.height ?? null, weight: player.weight ?? null, debut: player.mlbDebutDate ?? null, active: player.active ?? null,
        stats: splitsToObject(player.stats),
        situationalSplits: splitsToObject(splits.stats),
...
function normalizeRosterPlayer(entry) { const p = entry.person ?? {}; const statEntries = p.stats ?? []; return { id: p.id, name: p.fullName, position: entry.position?.name ?? p.primaryPosition?.name, number: entry.jerseyNumber ?? p.primaryNumber, status: entry.status?.description ?? null, stats: splitsToObject(statEntries) }; }
...
function normalizeGame(game) { return { type: "game", id: game.gamePk, name: `${game.teams?.away?.team?.name ?? "Away"} at ${game.teams?.home?.team?.name ?? "Home"}`, subtitle: `${game.officialDate ?? game.gameDate?.slice(0, 10)} · ${game.venue?.name ?? "Venue TBD"}`, date: game.gameDate ?? game.officialDate, officialDate: game.officialDate, venue: game.venue?.name ?? null, away: game.teams?.away?.team ?? null, home: game.teams?.home?.team ?? null, awayPitcher: game.teams?.away?.probablePitcher?.fullName ?? "TBD", homePitcher: game.teams?.home?.probablePitcher?.fullName ?? "TBD", weather: game.weather ? { condition: game.weather.condition ?? null, temperature: game.weather.temp ?? null, wind: game.weather.wind ?? null } : null, status: game.status?.detailedState ?? null, gameType: game.gameType ?? null, seriesDescription: game.seriesDescription ?? null, seriesGameNumber: game.seriesGameNumber ?? null, doubleHeader: game.doubleHeader ?? null, dayNight: game.dayNight ?? null, scheduledInnings: game.scheduledInnings ?? null, gamesInSeries: game.gamesInSeries ?? null, recordSource: "MLB Stats API" }; }
