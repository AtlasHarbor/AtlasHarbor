...
export function createProblemSpaceStorage({env=process.env,fetchImpl=globalThis.fetch}={}){
 ...
 async function patchUser(req,updater){const token=bearer(req);if(!token)throw signInError(token);return serial(`user:${token}`,async()=>{const fresh=await userForToken(token);if(!fresh)throw signInError(token);const patch=await updater(structuredClone(fresh.user_metadata||{}),fresh);if(!patch||typeof patch!=='object'||Array.isArray(patch))throw Object.assign(new Error('Account metadata patch is invalid.'),{status:500});const saved=await updateOwn(token,patch);return{patch,user:saved.user||saved}})}
 async function writeUser(req,space,updater){const token=bearer(req);if(!token)throw signInError(token);return serial(`user:${token}`,async()=>{const fresh=await userForToken(token);if(!fresh)throw signInError(token);const spaces={...(fresh.user_metadata?.atlas_problem_spaces||{})},value=await updater(structuredClone(spaces[space]||{}),fresh);spaces[space]=value;const metadata={...(fresh.user_metadata||{}),atlas_problem_spaces:spaces};const saved=await updateOwn(token,metadata);return{value,user:saved.user||saved}})}
 return{configured,bearer,userForToken,requestUser,listAccounts,hostAccount,readHost,updateHost,readGlobal,writeGlobal,readUser,patchUser,writeUser,readJson};
}
