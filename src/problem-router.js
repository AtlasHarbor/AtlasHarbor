...
export function createProblemRouter({service=createProblemSpacesService(),env=process.env}={}){
 ...
 const adminOk=req=>Boolean(env.ADMIN_PASSWORD)&&req.get('x-admin-password')===env.ADMIN_PASSWORD;
 ...
 router.get('/api/admin/problem-spaces',async(req,res)=>{if(!adminOk(req))return res.status(401).json({error:'Invalid admin password.'});try{return res.json({requests:await service.listAdmin()})}catch(error){console.error(error);return res.status(500).json({error:'Admin queue is temporarily unavailable.'})}});
 router.patch('/api/admin/problem-spaces/:id',async(req,res)=>{if(!adminOk(req))return res.status(401).json({error:'Invalid admin password.'});try{const row=await service.update(req.params.id,String(req.body?.status||''));return row?res.json({request:row}):res.status(404).json({error:'Request not found.'})}catch(error){return res.status(400).json({error:error.message})}});
 return router;
}
