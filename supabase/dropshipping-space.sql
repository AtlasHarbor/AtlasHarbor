...
create table if not exists public.publication_comments (
  ...
  workspace_note_id uuid not null references public.workspace_notes(id) on delete cascade,
...
create index if not exists publication_comments_note_idx on public.publication_comments(workspace_note_id,created_at);

create table if not exists public.dropship_strategies (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  author_alias text not null check (char_length(author_alias) between 2 and 80), title text not null check (char_length(title) between 3 and 140),
  product_name text not null check (char_length(product_name) between 2 and 160), product_url text, product_notes text not null default '',
  keywords text[] not null default '{}', platform text not null, geography text not null, audience_interests text[] not null default '{}',
  bid_strategy text not null default '', budget numeric, strategy_body text not null, ai_analysis text, results text,
  comments_enabled boolean not null default false, funding_open boolean not null default false,
...
create table if not exists public.strategy_comments (
  id uuid primary key default gen_random_uuid(), strategy_id uuid not null references public.dropship_strategies(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade, author_alias text not null check (char_length(author_alias) between 2 and 80),
  body text not null check (char_length(body) between 2 and 3000), is_ai_comment boolean not null default false, ai_model text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.strategy_funding_interest (
  ...
  user_id uuid not null references auth.users(id) on delete cascade, message text not null default '', created_at timestamptz not null default now(),
  primary key(strategy_id,user_id)
);
create table if not exists public.direct_threads (
  id uuid primary key default gen_random_uuid(), strategy_id uuid references public.dropship_strategies(id) on delete set null,
...
create table if not exists public.direct_thread_members (
  thread_id uuid not null references public.direct_threads(id) on delete cascade, user_id uuid not null references auth.users(id) on delete cascade,
  joined_at timestamptz not null default now(), primary key(thread_id,user_id)
);
create table if not exists public.direct_messages (
  id uuid primary key default gen_random_uuid(), thread_id uuid not null references public.direct_threads(id) on delete cascade,
  sender_id uuid not null references auth.users(id) on delete cascade, body text not null check (char_length(body) between 1 and 5000), created_at timestamptz not null default now()
);
...
alter table public.dropship_strategies enable row level security; alter table public.strategy_comments enable row level security;
alter table public.strategy_funding_interest enable row level security; alter table public.direct_threads enable row level security;
alter table public.direct_thread_members enable row level security; alter table public.direct_messages enable row level security;

grant select on public.publication_comments,public.dropship_strategies,public.strategy_comments to anon,authenticated;
grant insert,update,delete on public.publication_comments,public.dropship_strategies,public.strategy_comments to authenticated;
...
drop policy if exists "Public reads enabled publication comments" on public.publication_comments;
drop policy if exists "Users add enabled publication comments" on public.publication_comments;
create policy "Public reads enabled publication comments" on public.publication_comments for select using (exists(select 1 from public.workspace_notes n where n.id=workspace_note_id and n.is_shared=true and n.is_published=true and n.comments_enabled=true));
create policy "Users add enabled publication comments" on public.publication_comments for insert to authenticated with check (auth.uid()=user_id and exists(select 1 from public.workspace_notes n where n.id=workspace_note_id and n.is_shared=true and n.is_published=true and n.comments_enabled=true));
create policy "Users update own publication comments" on public.publication_comments for update to authenticated using (auth.uid()=user_id) with check (auth.uid()=user_id);
create policy "Users delete own publication comments" on public.publication_comments for delete to authenticated using (auth.uid()=user_id);
...
create policy "Thread creators read threads" on public.direct_threads for select to authenticated using (created_by=auth.uid() or exists(select 1 from public.direct_thread_members m where m.thread_id=id and m.user_id=auth.uid()));
create policy "Users create threads" on public.direct_threads for insert to authenticated with check (auth.uid()=created_by);
create policy "Users read own memberships" on public.direct_thread_members for select to authenticated using (user_id=auth.uid() or exists(select 1 from public.direct_threads t where t.id=thread_id and t.created_by=auth.uid()));
