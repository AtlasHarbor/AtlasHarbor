...
test('authoritative clock suppresses both legacy simulation timers',()=>{const js=read('../public/game-time-authority.js'),html=read('../public/game.html');assert.doesNotThrow(()=>new Function(js));assert.match(js,/REAL_MS_PER_GAME_HOUR=15\*60\*1000/);assert.match(js,/suppressed\.base/);assert.match(js,/advance/);assert.match(js,/suppressed\.command/);assert.match(js,/clockTick/);assert.match(js,/authority='network-v3-wall-anchor'/);assert.ok(html.indexOf('game-time-authority.js')<html.indexOf('game-v3.js'));assert.ok(html.indexOf('game-time-authority.js')<html.indexOf('game-command-center.js'))});

test('header is the only visible network time display',()=>{const html=read('../public/game.html'),css=read('../public/game-network-time.css'),clock=read('../public/game-time-authority.js');assert.match(html,/<div class="clock"><small>NETWORK TIME<\/small><b id="day">/);assert.match(css,/\.global-network-time\{display:none!important\}/);assert.match(clock,/document\.querySelector\('#day'\)/);assert.match(clock,/day\.textContent=formatHours\(nowHours,state\)/);assert.match(clock,/nativeSetInterval\(renderClock,500\)/)});
...
test('startup runs an eighteen-second historical catch-up replay before live pace',()=>{const js=read('../public/game-global-simulation-v2.js');assert.match(js,/const REPLAY_MS=18000/);assert.match(js,/const REPLAY_WINDOW_HOURS=24/);assert.match(js,/CATCH-UP REPLAY/);assert.match(js,/requestAnimationFrame\(replayStep\)/);assert.match(js,/finishReplay/);assert.match(js,/LIVE NETWORK/)});
