...
test('workspace has one database record with resilient transports and no device draft fallback',()=>{const source=read('../public/workspace.js');assert.match(source,/\/api\/workspaces\//);assert.match(source,/authenticated-session-metadata/);assert.match(source,/direct-account-database/);assert.match(source,/updateUserMetadata/);assert.match(source,/atlas_problem_spaces/);assert.match(source,/__atlasNativeFetch/);assert.ok(source.indexOf('sessionRow')<source.indexOf('freshAccount(),found'),'authenticated account metadata must be checked before a second network transport');assert.doesNotMatch(source,/localStorage\.(?:getItem|setItem).*workspace/);assert.doesNotMatch(source,/device copy|device draft/i)});
