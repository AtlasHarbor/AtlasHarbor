...
test('main game map is bridged and opens on the global network',()=>{const bridge=read('../public/game-map-bridge.js'),js=read('../public/game-management-v2.js'),html=read('../public/game.html');assert.match(bridge,/__atlasMainGameMap/);assert.match(js,/fitGlobal/);assert.match(js,/fitBounds\(globalBounds/);assert.match(js,/setMinZoom\(2\)/);assert.match(js,/renderMainMapOverlay/);assert.match(html,/Fit global network/);assert.match(html,/Focus active chapter/)});
