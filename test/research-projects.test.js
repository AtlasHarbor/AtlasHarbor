...
test('workspace recovery contract is documented',()=>{const docs=read('docs/WORKSPACE_ARCHITECTURE.md'),root=read('README.md');assert.match(docs,/Database first\. Recovery always\. Never database only\./);assert.match(docs,/workspace_notes/);assert.match(docs,/legacy_notes|legal_notes/);assert.match(docs,/sb_secret_/);assert.match(root,/docs\/WORKSPACE_ARCHITECTURE\.md/)});
